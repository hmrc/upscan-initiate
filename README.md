...
Assuming the consuming service is whitelisted, it makes a POST request to the `https://upscan-initiate.public.mdtp/upscan/initiate` endpoint. This request includes details about the expected upload, specifically the callback URL and optional constraints on size.

The callback will be made from inside the MDTP environment, bear that in mind when specifying the
callback URL e.g. `myservice.public.mdtp/upscan-callback` not `myservice.tax.gov.uk/upscan-callback`.
